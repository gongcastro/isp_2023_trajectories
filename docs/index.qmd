...
@floccia2018introduction, @mitchell2022cognates, @bosch2014first, @bilson2015semantic

:::

::: {.notes}
- Bilinguals learning languages that share more cognates show larger vocabulary sizes
...
::: notes
- Parallel activation refer to a phenomenon where bilinguals activate both languages even in monolingual situations
- The activation of a word in the lexicon may lead to the activation of its translation
- Cognates provide evidence of for the language-non selective hypothesis
- To date, the cognate facilitation has been embedded in models of word comprehension and production, but not acquisition
...
# Simulating bilingual word acquisition {background-color="#003f5c"}
...
data.frame(lambda = 50) %>%
    ...
    labs(title = "Frequency per month ~ Poisson(50)",
...
**`r format(nrow(distinct(responses)), big.mark = ",")` item responses** from **`r nrow(distinct(responses, id))` Catalan-Spanish bilinguals**
